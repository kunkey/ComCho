        ...
        if (isset($_POST['submit'])
            ...
            if ($datauser['postforum'] == 0) {
                if (!isset($_GET['yes'])) {
                    ...
                    echo '<div class="rmenu">' . $lng_faq['forum_rules_text'] . '</div>' .
                        '<div class="list4"><a href="index.php?act=say&amp;id=' . $id . '&amp;yes">' . $lng_forum['agree'] . '</a> | ' .
                        '<a href="index.php?id=' . $id . '">' . $lng_forum['not_agree'] . '</a></div>';
                    require('../incfiles/end.php');
                    exit;
                }
            }
    ...
    case 'm':
        ...
        */
        ...
        if (isset($_POST['submit'])
            ...
            // Thêm thông báo
            if (($kt == 1) && ($user_id != $type1['user_id'])) {
                $cha = mysql_fetch_assoc(mysql_query("SELECT `id`, `text` FROM `forum` WHERE `type` = 't' AND `id` = '" . $type1['refid'] . "'"));
                $nd = '[url=' . $set['homeurl'] . '/forum/index.php?id=' . $cha['id'] . '&page=' . $page . ']đã trích dẫn Bài viết của bạn tại Chủ đề: ' . $cha['text'] . '[/url]';
            ...
            if (($datauser['postforum'] == "" || $datauser['postforum'] == 0)) {
                if (!isset($_GET['yes'])) {
                    $lng_faq = core::load_lng('faq');
                    echo '<div class="rmenu">' . $lng_faq['forum_rules_text'] . '</div>';
                    echo '<div class="list4"><a href="index.php?act=say&amp;id=' . $id . '&amp;yes&amp;cyt">' . $lng_forum['agree'] . '</a> | <a href="index.php?id=' . $type1['refid'] . '">' . $lng_forum['not_agree'] . '</a></div>';
                    require('../incfiles/end.php');
                    exit;
                }
            }
