    ...
    */
    if ($do_file) {
        ...
        if (!$error) {
            ...
            echo '<div class="rmenu">' . functions::display_error($error, '<a href="index.php?act=addfile&amp;id=' . $id . '">' . $lng['repeat'] . '</a>') . '</div>';
        }
    } else {
        echo '<div class="rmenu">' . $lng_forum['error_upload_error'] . '</div>';
    }
    $pa = mysql_query("SELECT `id` FROM `forum` WHERE `type` = 'm' AND `refid` = '" . $res['refid'] . "'");
    $pa2 = mysql_num_rows($pa);
    $page = ceil($pa2 / $kmess);
    echo '<div class="list4"><a href="index.php?id=' . $res['refid'] . '&amp;page=' . $page . '">' . $lng['continue'] . '</a></div>';
} else {
    /*
    -----------------------------------------------------------------
